...
export class PerformancesService {
  ...
  async getPerformance(performanceId): Promise<Performance> {
    return await this.performanceModel.findById(performanceId).populate('pieces').exec();
  }

  async getPerformances(tourId: string): Promise<Performance[]> {
    return await this.performanceModel.find({tour: tourId}).populate('pieces').exec();
  }

  async editPerformance(performanceId, createPerformanceDTO: PerformanceDto): Promise<Performance> {
    return await this.performanceModel.findByIdAndUpdate(performanceId, createPerformanceDTO, {new: true}).populate('pieces');
  }
